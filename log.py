...
try:
    while True:
        ...
        if im.getpixel(left1) != derevo:
            ...
            # draw.text((width//2-200, height//2-200), 'GO LEFT', fill='red')
        ...
        if im.getpixel(right1) != derevo:
            ...
            # im = Image.open('temp.png')
            # draw = ImageDraw.Draw(im)
            # draw.text((width // 2 - 200, height // 2 - 200), 'GO RIGHT', fill='red')
            # im.save('log/' + str(cnt) + '.png')
            # cnt += 1
        # if browser.find_element_by_class_name("icon_refresh"):
        #     break
        # if im.getpixel(left) != derevo and im.getpixel(right) != derevo:
        #     browser.find_element_by_class_name("button_left").click()
        #     time.sleep(0.2)
        #     browser.save_screenshot('temp.png')
        #     im = Image.open('log/' + str(cnt) + '.png')
        #     draw = ImageDraw.Draw(im)
        #     draw.text((width // 2 - 200, height // 2 - 200), 'GO RANDOM (LEFT)', fill='red')
        #     im.save('log/' + str(cnt) + '.png')
        #     print('random')
